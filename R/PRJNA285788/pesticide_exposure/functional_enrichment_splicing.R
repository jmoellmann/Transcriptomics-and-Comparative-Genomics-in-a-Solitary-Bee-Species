...
for(treatment in c("imidacloprid", "thiacloprid")){
  for(ontology in c("CC", "MF", "BP")){
    ...
    out_dir <- paste0("04_results/PRJNA285788/pesticide_exposure/diff_splicing/", treatment, 
                      "/funct_enrich/", ontology)
    
    dir.create(out_dir, 
               showWarnings = FALSE, recursive = TRUE)
    
    res_all <- read_csv(paste0("04_results/PRJNA285788/pesticide_exposure/diff_splicing/", treatment, 
                        "/res_all.csv"))
