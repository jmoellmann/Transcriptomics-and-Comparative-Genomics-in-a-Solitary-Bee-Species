...
in_dir <- paste0(project_dir, "/results/PRJNA285788/pesticide_exposure/quantification/kallisto")
out_dir <- paste0(project_dir, "/results/PRJNA285788/pesticide_exposure/diff_expr/kallisto")
...
dds_wald <- de_analysis(in_dir, metadata, tx2gene, design = ~ Group, test = "Wald")

for(treatment in c("Thiacloprid", "Imidacloprid")){
  res <- tibble::as_tibble(results(dds_wald, contrast = c("Group", treatment, "Control"))) %>% 
    mutate(geneID = row.names(results(dds_wald, contrast = c("Group", treatment, "Control")))) %>% 
    arrange(geneID)
  ...
  write_csv(res, paste0("results/PRJNA285788/pesticide_exposure/diff_expr/kallisto/DESeq2_results_", 
