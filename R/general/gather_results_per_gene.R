...
male_biased_genes <- ob_genes %>% filter(singleCopyOrthl, pvalue < 0.05, log2FoldChange > 0)
female_biased_genes <- ob_genes %>% filter(singleCopyOrthl, pvalue < 0.05, log2FoldChange < 0)
...
sex_biased_selection_test <- binom.test(x = nrow(male_biased_selected_genes), 
           ...
           p = prob_male_biased, alternative = "greater")
...
deg_enrichment_test_matrix <- matrix(data = c(nrow(ob_genes %>% filter(singleCopyOrthl)), 
                               ...
                               nrow(ob_genes %>% filter(singleCopyOrthl, pvalue < 0.05)),
                               nrow(ob_genes %>% filter(episDivSelection, pvalue < 0.05))), nrow = 2)
